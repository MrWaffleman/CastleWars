...
public class Utils {
	...
	public int createArena (CommandSender creator, String[] args) {
		...
		boolean used = false;
		for (String s :parent.getArenasConfig().getConfig().getStringList("arenas")) {
		...
		if(used) {
			...
			Selection selection = parent.getWorldedit().getSelection((Player)creator);
			...
			if(selection != null) {
				if(selection instanceof Polygonal2DSelection) {
					...
					if(CastleWars.Verbose) {
						parent.getLogger().log(Level.INFO, creator.getName() + " CREATED ARENA NAME: " +args[0] );
					}
