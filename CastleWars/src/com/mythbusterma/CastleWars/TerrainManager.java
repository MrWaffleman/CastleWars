 ...
 * @author desht
 * 
 *         A wrapper class for the WorldEdit terrain loading & saving API to
 *         make things a little simple for other plugins to use.
 * 
 */
public class TerrainManager {
	...
	public TerrainManager(WorldEditPlugin wep, World world) {
		...
		editSession = new EditSession(new BukkitWorld(world),
				we.getConfiguration().maxChangeLimit);
	}

	private Vector getMax(Location l1, Location l2) {
		return new Vector(Math.max(l1.getBlockX(), l2.getBlockX()), Math.max(
				l1.getBlockY(), l2.getBlockY()), Math.max(l1.getBlockZ(),
				l2.getBlockZ()));
	}

	private Vector getMin(Location l1, Location l2) {
		return new Vector(Math.min(l1.getBlockX(), l2.getBlockX()), Math.min(
				l1.getBlockY(), l2.getBlockY()), Math.min(l1.getBlockZ(),
				l2.getBlockZ()));
	}
	 ...
	 * @throws EmptyClipboardException
	 */
	public void loadSchematic(File saveFile) throws FilenameException,
			DataException, IOException, MaxChangedBlocksException,
	 ...
	 * 
	 * @param saveFile
	 *            a File representing the schematic file to load
	 * @param loc
	 *            the location to paste the clipboard at (may be null)
	 * @throws FilenameException
	 * @throws DataException
	 * @throws IOException
	 * @throws MaxChangedBlocksException
	...
	public void loadSchematic(File saveFile, Location loc)
		...
		saveFile = we.getSafeSaveFile(localPlayer, saveFile.getParentFile(),
				saveFile.getName(), EXTENSION, new String[] { EXTENSION });

		editSession.enableQueue();
		localSession.setClipboard(SchematicFormat.MCEDIT.load(saveFile));
		localSession.getClipboard().place(editSession, getPastePosition(loc),
				false);
	 ...
	 * 
	 * @param saveFile
	 *            a File representing the schematic file to create
	 * @param l1
	 *            one corner of the region to save
	 * @param l2
	 *            the corner of the region to save, opposite to l1
	 * @throws DataException
	 * @throws IOException
	 */
	public void saveTerrain(File saveFile, Location l1, Location l2)
		...
		editSession.enableQueue();
		CuboidClipboard clipboard = new CuboidClipboard(max.subtract(min).add(
				new Vector(1, 1, 1)), min);
